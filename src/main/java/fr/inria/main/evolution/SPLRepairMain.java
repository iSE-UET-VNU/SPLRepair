...
import fr.inria.astor.approaches.splgenprog.SPLGenProg;
import fr.inria.astor.core.entities.OperatorInstance;
import fr.inria.astor.core.entities.ProgramVariant;
import fr.inria.astor.core.setup.FinderTestCases;
import fr.inria.astor.core.solutionsearch.navigation.FailingProductNavigationStrategy;
import fr.inria.astor.core.solutionsearch.navigation.UniformRandomFailingProductNavigation;
...
public class SPLRepairMain extends AbstractMain {
    ...
    public SPLSystem run_splrepair(String location, String projectName, String dependencies, String packageToInstrument,
        ...
        String failingproductnavigation = ConfigurationProperties.getProperty("failingproductnavigation").toLowerCase();
        FailingProductNavigationStrategy failing_product_navigator = get_failing_product_nagivator(failingproductnavigation);
        if(failing_product_navigator == null){
            log.error("FailingProductNavigationStrategy");
            return null;
        }
        int generation = 0;
        boolean stopSearch = false;
        int maxgeneration = Integer.parseInt(ConfigurationProperties.getProperty("maxgenerationsplsystem"));
        while (generation < maxgeneration && !stopSearch) {
            log.info("**** SPL generation: " + generation);
    ...
    private FailingProductNavigationStrategy get_failing_product_nagivator(String failingproductnavigation){
        if(failingproductnavigation.equals("random")){
            return new UniformRandomFailingProductNavigation();
        }
        return null;
    }
